...
p := &Parser{l: l}
errors: []string{},      // Read two tokens, so curToken and peekToken are both set
p.nextToken() 
p.nextToken()
...
func (p *Parser) noPrefixParseFnError(t token.TokenType) {
	msg := fmt.Sprintf("no prefix parse function for %s found", t) p.errors = append(p.errors, msg)
	}
...
if p.peekTokenIs(token.RPAREN) { p.nextToken()
return identifiers 
}
p.nextToken()
